...
function selectedFixationSystem(type, fixationSystemInputs, parentSection) {
  let price = 0
  let fixationSystemParentBlock =
    fixationSystemInputs[0].closest('.fixationSystem')
    ...
    fixationSystemInputs.forEach(function (radio) {
      if (type == 'COMPACT' || type == 'MINI' || type == 'STANDART') {
        if (radio.value == 'Направляющая струна') {
          ...
          radio.parentNode.querySelector('span').textContent =
            'Направляющая леска'
        }
      } else {
...
function selectedChainFixingUniversal(type, chainLoad) {
  let price = 0
  let chainFixingUniversaParentBlock = chainLoad[0].closest(
...
function viewControlMethod(
  ...
  let controlMethodChanel = document.querySelector(
    parentSection + ' .controlMethodChanel'
  )
  let quantityRemotes = document.querySelector(
    parentSection + ' .quantityRemotes'
  )
  let chargingRemotes = document.querySelector(
    parentSection + ' .chargingRemotes'
  )
...
function viewChanelControlMethod(
  controlMethod,
  chanels,
  remotes,
  charging,
...
function priceControlMethod(type, boxControlMethod) {
  ...
  let controlMethodChanel = boxControlMethod.querySelector(
    '.controlMethodChanel select'
  )
  let quantityRemotes = boxControlMethod.querySelector(
    ' .quantityRemotes select'
  )
  let chargingRemotes = boxControlMethod.querySelector(
    ' .chargingRemotes select'
  )
...
function viewEaxtraWidth(type, parentSection) {
  let mainWidthText = document.querySelector(
    parentSection + ' .main_width span'
  )
...
function viewAdditionalScotchTape(type, parentSection) {
  let additionalScotchTape = document.querySelector(
    parentSection + ' .additionalScotchTape'
  )
