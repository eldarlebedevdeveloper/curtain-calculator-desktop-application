...
function decreaseInTotalCost(allProducts) {
  ...
  allProducts.forEach(function (prod) {
    if (
      prod.idParent === 'roller_curtains' ||
      prod.idParent === 'roller_curtains_day_night' ||
      prod.idParent === 'roller_curtains_ready'
    ) {
    ...
    if (
      prod.idParent === 'new_vertical_89mm' ||
      prod.idParent === 'new_vertical_127mm'
    ) {
...
function viewTotalCostInDisplay(totalCost, id) {
  ...
  let allDisplayElementHryvnia = document.querySelectorAll(
    '[data-totalCostHryvnia]'
  )
